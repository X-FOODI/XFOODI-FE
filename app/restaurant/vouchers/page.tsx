...
function runStatus(v: Voucher): { label: string; cls: string } {
  if (v.status !== 'active')
    return { label: 'Bị khóa', cls: 'text-red-400 bg-red-400/10 border-red-400/20' };
  const now = Date.now();
  const s = safeDate(v, 'start');
  const e = safeDate(v, 'end');
  if (now < s)
    return { label: 'Chờ hiệu lực', cls: 'text-yellow-400 bg-yellow-400/10 border-yellow-400/20' };
  if (now > e)
    return { label: 'Hết hiệu lực', cls: 'text-orange-400 bg-orange-400/10 border-orange-400/20' };
  return { label: 'Đang chạy', cls: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' };
}
...
export default function OwnerVouchersPage() {
  ...
  // ── Auth guard ──────────────────────────────────────────────────────────────
  // Tạm thời comment out auth guard để test nhập liệu dưới local
  // useEffect(() => {
  //   if (isAuthReady && !user) router.replace('/login?redirect=/restaurant/vouchers');
  ...
  const fetchVouchers = useCallback(async (restaurantId: string) => {
    ...
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/vouchers/restaurant/${restaurantId}`);
  ...
  const handleModalSubmit = async (data: VoucherFormData) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        expiryDate:      data.endDate,                // ISO date string từ <input type="date">
  ...
  // ── Loading guard ───────────────────────────────────────────────────────────
  // Tạm thời comment loading guard để test local khi chưa đăng nhập
  // if (!isAuthReady || !user) {
  //   return (
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6">
          <div className="max-w-[1400px] mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-black text-white">Mã Khuyến Mãi</h1>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={openCreate}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-orange-500 text-gray-900 font-black text-sm hover:bg-orange-400 transition-colors"
                >
            ...
            <div className="p-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] space-y-3">
              ...
              <div className="flex justify-between items-center text-xs text-zinc-500">
                ...
                <select
                  ...
                  className="bg-transparent text-zinc-400 focus:outline-none text-xs"
                >
                  <option value="newest">Mới nhất</option>
                  <option value="end_soon">Hết hạn sớm</option>
                  <option value="end_late">Hết hạn muộn</option>
                  <option value="usage_ratio">Dùng nhiều nhất</option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
              {loading ? (
                ...
                <div className="text-center py-12 text-zinc-500">
                  <p className="font-bold">Chưa có mã nào</p>
                  <button
                    onClick={openCreate}
                    className="mt-3 text-orange-400 text-sm font-bold hover:underline"
                  >
                    + Tạo mã đầu tiên
                  </button>
                </div>
                    ...
                    <thead>
                      <tr className="border-b border-white/[0.08]">
                        {[
                          ...
                          <th
                            key={h}
                            className="text-left px-4 py-3 text-[10px] font-black uppercase tracking-wider text-zinc-500"
                          >
                            {h}
                          </th>
                        ))}
                      ...
                      {filtered.map((v) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-zinc-300 text-xs">{venueLabel}</td>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() => openEdit(v)}
                                  className="p-1.5 rounded-lg text-zinc-400 hover:text-teal-400 hover:bg-teal-400/10 transition-colors"
                                  title="Chỉnh sửa"
                                >
                                  <Edit2 size={13} />
                                </button>
                                <button
                                  ...
                                  className="p-1.5 rounded-lg text-zinc-400 hover:text-red-400 hover:bg-red-400/10 transition-colors disabled:opacity-30"
                                  title={
                                    (v.usedCount ?? 0) > 0
